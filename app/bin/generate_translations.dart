  ...
  for (final row in rows.sublist(1)) {
    ...
    for (var i = 0; i < languageCodes.length; i++) {
      ...
      final unusedParameters = methodSignature.parameters.map((parameter) => parameter.name).toSet()
          ..removeAll(translation.parts.whereType<VariablePart>().map((variable) => variable.name));
...
''')
      ..writeln(abstractBaseClass.toDartCode())
      ..writeln('')
      ..writeln(delegateClass.toDartCode());
  for (final languageClass in languageClasses.values) {
    output
        ..writeln('')
        ..writeln(languageClass.toDartCode());
  }
  output.close();
}
...
class Translation {
  ...
  static VariableType _typeFromChar(String char) {
    switch (char) {
      case r'$': return VariableType.string;
